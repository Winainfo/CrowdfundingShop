...
#import "InvitationRecordController.h"
#import "InvitationRecordCell.h"
#import "RequestData.h"
#import "AccountTool.h"
#import <UIImageView+WebCache.h>
#import <MBProgressHUD.h>
#import <MJRefresh.h>
@interface InvitationRecordController (){
...
@implementation InvitationRecordController
...
- (void)viewDidLoad {
    ...
    //创建xib文件对象
    UINib *nib=[UINib nibWithNibName:@"InvitationRecordCell" bundle:[NSBundle mainBundle]];
    //注册到表格视图
    [self.myTableView  registerNib:nib forCellReuseIdentifier:@"InvitationRecordCell"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *cellStr=@"InvitationRecordCell";
    InvitationRecordCell *cell=[self.myTableView dequeueReusableCellWithIdentifier:cellStr];
    if (cell==nil) {
        cell=[[InvitationRecordCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellStr];
    }
    if ([_goodsArray[indexPath.row][@"username"]isEqualToString:@""]) {
        NSString *phone=_goodsArray[indexPath.row][@"mobile"];
        NSString *phoneString = [phone substringWithRange:NSMakeRange(5,4)];
        phone=[phone stringByReplacingOccurrencesOfString:phoneString withString:@"****"];
        [cell.userButton setTitle:[NSString stringWithFormat:@"%@",_goodsArray[indexPath.row][@"sqlname"]] forState:UIControlStateNormal];
    }else{
      [cell.userButton setTitle:[NSString stringWithFormat:@"%@",_goodsArray[indexPath.row][@"username"]] forState:UIControlStateNormal];
    }
    //时间戳转换时间
    long time=[_goodsArray[indexPath.row][@"time"] integerValue];//时间戳
    cell.timeLabel.text=[InvitationRecordController timeFromTimestamp:time formtter:@"yyyy.MM.dd HH:mm:ss"];
    cell.codeLabel.text=[NSString stringWithFormat:@"%@",_goodsArray[indexPath.row][@"yaoqing_code"]];
    cell.stateLabel.text=[NSString stringWithFormat:@"%@",_goodsArray[indexPath.row][@"joinin"]];
    //取消Cell选中时背景
