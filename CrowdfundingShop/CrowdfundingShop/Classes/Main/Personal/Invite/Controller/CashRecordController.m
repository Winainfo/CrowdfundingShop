...
#import <MJRefresh.h>
@interface CashRecordController (){
...
- (void)viewDidLoad {
    ...
    //创建xib文件对象
    UINib *nib=[UINib nibWithNibName:@"CashRecordCell" bundle:[NSBundle mainBundle]];
    //注册到表格视图
    [self.myTableView  registerNib:nib forCellReuseIdentifier:@"CashRecordCell"];
...
-(void)loadNewData
{
    if (self.goodsArray.count==0) {
        ...
        //显示的文本；
        hud.labelText = @"正在加载...";
        [RequestData cashRecordSerivce:params FinishCallbackBlock:^(NSDictionary *data) {
            NSLog(@"---%@",data);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *cellStr=@"CashRecordCell";
    CashRecordCell *cell=[self.myTableView dequeueReusableCellWithIdentifier:cellStr];
    if (cell==nil) {
        cell=[[CashRecordCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellStr];
    }
    //时间戳转换时间
    long time=[_goodsArray[indexPath.row][@"time"] integerValue];//时间戳
    cell.timeLabel.text=[CashRecordController timeFromTimestamp:time formtter:@"yyyy-MM-dd"];
    cell.moneyLabel.text=_goodsArray[indexPath.row][@"money"];
    cell.stateLabel.text=_goodsArray[indexPath.row][@"status"];
    //取消Cell选中时背景
